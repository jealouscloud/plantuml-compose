...
class TestUseCaseComposer:
    ...
    def test_render_matches_builder(self):
        """Simple actors + usecases + arrow matches old builder."""
        from plantuml_compose.builders.usecase import (
            usecase_diagram as builder_usecase,
        )

        # Old builder
        with builder_usecase(title="Test") as old:
            user = old.actor("User")
            browse = old.usecase("Browse")
            old.arrow(user, browse)
    ...
    def test_render_matches_builder_generalizes(self):
        """Generalizes relationship matches old builder."""
        from plantuml_compose.builders.usecase import (
            usecase_diagram as builder_usecase,
        )

        # Old builder
        with builder_usecase() as old:
            engineer = old.actor("Engineer")
            oncall = old.actor("OnCall", alias="oncall")
            old.generalizes(oncall, engineer, direction="up")
        old_output = render(old.build())
        ...
        r = d.relationships
        engineer = el.actor("Engineer")
        oncall = el.actor("OnCall", ref="oncall")
        d.add(engineer, oncall)
        d.connect(r.generalizes(oncall, engineer, direction="up"))
        new_output = render(d)
    ...
    def test_render_matches_builder_include(self):
        """Include relationship matches old builder."""
        from plantuml_compose.builders.usecase import (
            usecase_diagram as builder_usecase,
        )

        # Old builder: requires(base, required) -> source=base, target=required
        with builder_usecase() as old:
            checkout = old.usecase("Checkout")
            validate = old.usecase("Validate")
            old.requires(checkout, validate)
        old_output = render(old.build())
